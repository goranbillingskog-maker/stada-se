...
export function companiesForService(serviceSlug, citySlug = null) {
  ...
  const localList = base.filter((c) => matchesService(c, service));

  // If flyttstadning on Södermalm: supplement with verified nearby partners with explicit Södermalm/innerstan coverage
  if (serviceSlug === "flyttstadning" && citySlug === "sodermalm") {
    const extraPartners = [
      { citySlug: "nacka", slug: "di-mas-ab", coverageNote: "Tar uppdrag på Södermalm" },
      { citySlug: "vasastan", slug: "stadfabriken-ab", coverageNote: "Tar uppdrag på Södermalm" },
    ];
    const all = getAllCompanies();
    const extras = extraPartners
      .map(({ citySlug: cs, slug, coverageNote }) => {
        const found = all.find((c) => c.citySlug === cs && c.slug === slug);
        return found ? { ...found, coverageNote } : null;
      })
      .filter(Boolean);

    const combined = [...localList];
    for (const ex of extras) {
      if (!combined.some((c) => c.slug === ex.slug)) {
        combined.push(ex);
      }
    }
    return combined;
...
export const CITY_COORDS = {
  ...
  sodermalm: [59.31, 18.07],
  vasastan: [59.34, 18.04],
  ostermalm: [59.34, 18.08],
  ...
  lidingo: [59.366, 18.133],
  sundbyberg: [59.362, 17.973],
  danderyd: [59.412, 18.053],
  jarfalla: [59.417, 17.833],
  huddinge: [59.237, 17.982],
  botkyrka: [59.1983, 17.8344],
  haninge: [59.1678, 18.1461],
  tyreso: [59.2444, 18.2325],
  "upplands-vasby": [59.5197, 17.9103],
  sigtuna: [59.6173, 17.7236],
